...
class Marking_Data:
    ...
    def get_file(self, file):  # Function to get the cli file
        try:
            self.file = file
        except Exception as e:
            self.main.parent_connection.send(f"Error C2: Error in cli_file_handline.get_file()\n{str(e)}")

    def checkForErrors(self): # Function that checks the cli file for errors and format
        pass

    def generate_data(self):   # Function to generate the data structure that holds all the details obtained from the cli file
        try:
    ...
    def initialise_print(self):     # Function to create a thread for this purpose
        try:
    ...
    def start_printing_operation(self):  # Function to get the next point, make the command and send it to E1803( still incomplete and not tested)
        try:
            self.payload = []
            coord = ""
    ...
    def start_print_temp(self): # Function to go through the data structure and get points one by one
        print("Debug 1")
        try:
    ...
    def get_next_pair(self):  # Function to get the next pair of coordinates from the data structure
        try:
            if self.layer_key_index < len(self.layer_key_list):
