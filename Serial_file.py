...
class Serial_Class(QObject):  # Class to handle serial communication
    ...
    def connect(self):  # Function to automatically connect to the tiva board if found
        try:
            ...
            try:
                ...
                if len(data) > 1:  # Because all the return statements length is greater than 5, used to avoid reading of garbage value
                    ...
                    if "Ex" in data or " L1" in data or "L2" in data:        #Also, to account for non-linear motion of recoater               #Because recoater sends ok instantly but will be in loop and will send Ex.. after the loop
                        self.connection.send(data)        #For waitforResponse function
                    # if self.parent.debug_var == 2:
                    #     print(data)
...
class SerialMonitor(QDialog):  #Tried to do something but don't remember and it is not used and does not affect the operation in any way
    def __init__(self):
        super().__init__()
