...
R_Commands = ["Roller_FW", "Roller_RV", "Roller_STOP_IN", "Roller_STOP_DC", "Roller_START", "Roller_RUN_FW",
              ...
              "Recoater_Go_Left", "Recoater_Go_Right", "Recoater_RPM","reset","gpio_reset"]  # All recoater system commands

H_Commands = ["move_hopper_right","move_hopper_left"]       # Hoppper commands
...
class GUI(QMainWindow, QObject):
    ...
    def __init__(self, parent_conn, child_conn, scanner_parent_conn, scanner_child_conn):
        try:
            ...
            self.serial = Serial_Class(self)     # Instance of serial class
            self.time_instant = Time(self)        # Instance of time class
            self.cli_file_object = Marking_Data(self)      # Instance of ..
            self.scancard = ScanCard() #" "

            # Scanner connection objects initialisation
            self.scanner_parent_conn = scanner_parent_conn       #Multipprocessing/SlotsandConnections pipes got from start of the program for scanner purpose
            self.scanner_child_conn = scanner_child_conn

            self.file_path = ""                             # No idea
            self.WFR_previous_data = ""

            self.debug_var = 1                             # Variable used for debugging
            ...
            self.parent_connection = parent_conn              # Multiprocessing pipe
            ...
            self.laser_set.released.connect(self.change_laser_parameters)
            self.laser_set_stdby.released.connect(self.change_laserStdBy_parameters)
            ...
            self.laser_set.pressed.connect(lambda: self.ButtonClickEffect(self.laser_set))
            self.laser_set_stdby.pressed.connect(lambda: self.ButtonClickEffect(self.laser_set_stdby))


            #Effect, not enabled because idk why but I don't want to import main to scancard.py , can be added by routing a flow to a local function and after updation of the widget we can call the function in scancard.py
            ...
            # self.instStop.clicked.connect(lambda: self.instantStop(True)) #Not used currently, but I have put the put the button somewhere in the gui
    ...
    def change_laserStdBy_parameters(self): #Function to modify laser parameters during jump( when not marking)  FOR E1803
        try:
            self.laser_set_stdby.setStyleSheet(self.current_style)      #resetting the stylesheet of the button to original one
        except Exception as e:
            print(str(e))
        ...
        try:
            global LASER_MODE
            if LASER_MODE:
                self.scancard.send_command(b'd\x0c\x00\x00\x00\x48\x00\x00\x00\x00\x00\x00\x00\x00')         # Command to select CO2 laser in E1803
                #print("response:",self.receive_response())
                LASER_MODE = False

            laser_stdby_frequency = int(self.laser_stdby_frequency.text())
            pulse_stdby_width = int(self.pulse_stdby_width.text())

            if laser_stdby_frequency < 25 or laser_stdby_frequency > 20000000 or pulse_stdby_width < 1 or pulse_stdby_width > 65530:  #Limits from the manual
                print("Please enter appropriate values, in the range of [25,20000000] of LF and [1,65530] for PW")
            else:
                cmd_ = self.scancard.make_command(laser_stdby_frequency,pulse_stdby_width,0,"laser_para")   # Calling a function in scancard.py
                self.scancard.send_command(cmd_)
        except Exception as e:
            print(str(e))

    def change_laser_parameters(self): # Function to modify laser parameters when marking       FOR E1803
        try:
            self.laser_set.setStyleSheet(self.current_style)          #Button click effect
        except Exception as e:
            print(str(e))

        try:
            global LASER_MODE
            if LASER_MODE:          # The flow will go in only when this function is called for the first time
                print("laser")
                self.scancard.send_command(b'd\x0c\x00\x00\x00\x48\x00\x00\x00\x00\x00\x00\x00\x00')     # will set it to CO2 laser mode
                LASER_MODE = False

            laser_frequency = int(self.laser_frequency.text())
            pulse_width = int(self.pulse_width.text())

            if laser_frequency < 25 or laser_frequency > 20000000 or pulse_width < 1 or pulse_width > 65530:
                print("Please enter appropriate values, in the range of [25,20000000] of LF and [1,65530] for PW")
            else:
                print("laser para")
                cmd_ = self.scancard.make_command(laser_frequency,pulse_width,0,"laser_para")
                self.scancard.send_command(cmd_)
        except Exception as e:
            print(str(e))


    def JumpToXYZ(self):      # function to jump the scanner to a give x,y, and z coordingate
        try:
    ...
    def MarkToXYZ(self): # function to mark the coordinate x,y and z
        try:
    ...
    def testing_function(self):          # Used for temporary testing requirements, the button is present in the second page of the GUI
        test_thread = threading.Thread(target = test.testing_p,args = (self,))      # Will create a separate thread for this
        test_thread.daemon = True
        test_thread.start()

    def change_scanner_speed(self):  # Function to change scanner attributes    FOR E1803
        try:
            markspeed = int(self.markspeed.text())
            jumpspeed = int(self.jumpspeed.text())
            ...
            print(str(e))
    def rotate_log(self):     # The idea was to have log file record all the modificatios and changes and progress
        pass

    def RwaitforResponse(self):  # Had initially written for waiting for response after trapezoidal motion of recoater. Used only during synchrrnous process
        while True:
    ...
    def waitforResponse(self,req = None):   #Functin to wait for a response from tiva before proceeding to next commands
        while True:
           if not self.parent_connection.poll():   # Checks if data is present in the multiprocessing pipe
                continue
           else:
                received = self.parent_connection.recv()
                if req == None and ("Ex" in received or "L1" in received or "L2" in received):    # either Ex ack from tiva or limit switch clicked
    ...
    def waitforPause(self):           # During the synchronized process this function is used to check whether the pause button is clicked
        if self.pause_flag.value == True:
            while True:
                with self.semaphore:   # Acquiring semaphore
                    if self.pause_flag.value == False:
                        break

    def stop_scanning_operation(self):      # Function is called when stop print is clicked from CLI page in the GUI ( FOR E1803)
        self.stop_print.setStyleSheet(self.current_style)
        self.scanner_parent_conn.send("finish")

    def open_file_dialog(self):       # This function openst the file manager
    ...
    def cli_final_select(self):                #this function is called when you select a cli file and click ok
        ...
        try:
            ...
            if not self.cli_file_object.checkForErrors():   #User defined function in cli_file_handling.py ( Not yet written, just defined)
                self.cli_file_object.generate_data()        # Will generate the data structure that contains the data acquired from the CLI file. It is an UDF in cli_file_handling.py
        except Exception as e:
            # print(e)
            self.parent_connection.send(f"Error M3: Error in cli_final_select()\n{e}")

    def get_next_coordinates(self):         # Function to get the next coordinates from the cli file
        self.get_point.setStyleSheet(self.current_style)
        try:                                 # Go through cli_file_handling.py for better understanding
    ...
    def page_change(self, page):         # Function to change the pages in the gui - stackedWidget
        try:
            if page == "dashboard":
    ...
    def ButtonClickEffect(self, object): # Function that gives the effect of the button getting clicked
        ...
        try:
            ...
            for line in current_style.split(';'):  # Checking for background-color line
                if line.strip().startswith("background"):  # Getting the line that corresponds to background color attribute from .ui file
                    line = line.split(":")
                    line[1] = line[1].strip()
                    rgb = line[1][4:len(line[1]) - 1].split(",")      # Getting the RGB values
                    # print(rgb)
                    red = int(rgb[0])
                    green = int(rgb[1])
                    try:
    ...
    def pause_auto_process(self):  # Function to pause the automation loop
        try:
            ...
            with self.semaphore:  # Acquiring the semaphore
                if self.pause_flag.value == True:       # Checking whether the process is to paused or resumed
    ...
    def start_thread(self):  # Function to initialise and start the automation loop
        ...
        try:
            if self.debug_var == 2:      # When debug level 2 is selected, more information is displayed to the user, you can add  info like this
                self.printToConsole("Creating a new process")
                print("Creating a new process")
            self.stop_event = threading.Event()   # Event to handling the start and stop of the new process
            self.auto_process = multiprocessing.Process(target=self.start_process_function)    # Creating a new process for the synchronous process
    ...
    def start_process_function(self):  # Function to handle the automation loop  # I suggest to write fresh
        try:
           # cmd = "Ready\r"  # Dummy command to the tiva board. Tiva board does not react
            #self.serial.serial_write(cmd)
            #time.sleep(2)

            hopper_cmdL = {                     # Left hopper command
                "script": "move_hopper_left"
            }

            hopper_cmdR = {                     # Right hopper command
                "script": "move_hopper_right"
            }

            # Setting RPM of Recoater motors
            self.RPM(True, 1750, "auto")          # Setting RPM(Recoater) to 101, i.e, the default will be 100(in python and not in tiva) and it will do 100 + 1.
            self.waitforResponse()
            self.RPM(False, 146, "auto")          # Setting RPM(Roller) to 101, i.e, the default will be 100(in python and not in tiva) and it will do 100 + 1.
            self.waitforResponse()

            self.waitforPause()

            self.start(False)                   # Start Roller
            self.waitforResponse()              # Wait for response from Tiva
            self.start(True)                    # Start Recoater
            #self.waitforResponse(["L1","L2"])     #Pass arguments to check for specific response like limit switch #Check whether limit switch was clicked
            self.waitforResponse()
        ...
        try:
            while not self.stop_event.is_set():  # auotmation loop. order to operation = Z -> Hopper -> Recoater
                try:
                    if self.pause_flag.value == True:
                        ...
                        self.RwaitforResponse()    #Put it back because i removed it from the changeDirection function becuase the whole gui was freezing, here it is fine becuase it is a separate thread
                        try:
                            if self.debug_var == 2:
                                print("Moving z")
            ...
            self.stop_event.clear()                     # To make sure the next call of this functions executes properly and gets into the while loop
        except Exception as e:
    ...
    def environment_control(self):  # Function to handle the environment management process
        try:
            while True:
                self.lock.acquire()
                self.temperature = self.environment_instance.update_screen()   # acquiring temperature data
                self.lock.release()
                time.sleep(5)
        except Exception as e:
            self.parent_connection.send(f"Error M19: Error in environment_control(): {e}")

    def update_gui_temp(self, status):  # Function to update the temperature values in the GUI
        # Current Temperature
        try:
            if "tiva" in status.keys():
                if status["tiva"] == "Online":
                    self.tiva_status.setText("Tiva Online")
                    self.tiva_status.setStyleSheet("color: green")        # Will update tiva status in gui
                else:
            ...
            if "Temperature_data" in status.keys():
                ...
                if temperatures != "":
                    ...
                    # Check the name of the termistors and the sub keys as well to get correct values. Run the this part in the separate file and print it

                    # Heater state display in the gui
                    try:
    ...
    def R_motion_attributes(self):   # This function was written to change the pitch, rpm etc of the trapezoidal motion on the recoater but it is not being used now. To use need to update Tiva code also
        try:
            self.set_attributes.setStyleSheet(self.current_style)
            global PITCH, GEAR_RATIO, FAST_RPM_DISTANCE, SLOW_RPM_DISTANCE, FAST_RPM, SLOW_RPM  # Default values
            try:
    ...
    def R_disable_buttons(self):# This function is called to disable recoater button during trapezoidal motion of the recoater
        try:
            self.start_process.setEnabled(False)
            self.recoaterRPM.setEnabled(False)
            self.rollerRPM.setEnabled(False)
    ...
    def R_enable_buttons(self): # Function to enable the recoater buttons after the trapezoidal motion
        try:
            self.start_process.setEnabled(True)
            self.recoaterRPM.setEnabled(True)
            self.rollerRPM.setEnabled(True)
    ...
    # def printToConsole(self, text):  # Function to print to the console in the UI ( Old version, here I couldn't add colors)
    #     #print("Hi Console")
    ...
    def printToConsole(self, text):  #Function to print to the console with colors
        # print("Hi Console")
        # Add text to the console
        try:
    ...
    def printToSerialMonitor(self, text):  # Function to print to the serial console in the UI
        ...
        try:
            ...
            if rpm == True and num == 0:  # When RPM is set using line edit widget, i.e entering the value for MOTOR 1
                ...
                if val.isdigit():
                    ...
                    if val < 40 or val > 3150:     # To vary the range of values accepted by the program
        ...
        try:
            ...
            if motor_select:  # Motor 1
                try:
                    ...
                    print(f"Error! Button click effect error. Might be due to synchronous process")
    ...
    def dynamic_commands(self):  # Function to type defined commands in the console and send it to octopus board (type in Console input not serial input)
